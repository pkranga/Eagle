...
@Component({
  selector: 'app-quarte-filter',
  templateUrl: './quarte-filter.component.html',
  styleUrls: ['./quarte-filter.component.scss']
})
export class QuarteFilterComponent implements OnInit {
    ...
    try {
      ...
      if (quarter) {
        ...
        if (quarter && this.config.selectedYear === null) {
          this.config.selectedYear = this.config.years[this.config.years.length - 1].value;
        }
        if (type === 'add') {
