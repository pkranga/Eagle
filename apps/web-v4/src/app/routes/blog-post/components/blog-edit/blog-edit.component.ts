  ...
  publishBlog(publishMsg: string, errorMsg: string) {
    ...
    if (this.editMode === 'draft') {
      ...
      this.postPublishRequest.source = {
        id: `${this.configSvc.instanceConfig.platform.rootOrg}_${this.configSvc.instanceConfig.platform.org}_SocialForum`,
        name: 'SocialForum'
      }
    }
    ...
    this.socialSvc.publishPost(this.postPublishRequest).subscribe(
      data => {
        ...
        if (data && data.id) {
          ...
          this.router.navigate(['blog-post', 'me', 'published']);
        }
      },
      err => {
  ...
  saveDraft(successMsg: string, errorMsg: string) {
    ...
    this.socialSvc.draftPost(request).subscribe(
      data => {
        ...
        this.isCreatingPost = false;
        this.router.navigate(['blog-post', 'me', 'draft']);
      },
      err => {
        this.openSnackBar(errorMsg);
        this.isCreatingPost = false;
