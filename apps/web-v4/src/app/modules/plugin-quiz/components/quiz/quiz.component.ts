...
export class QuizComponent implements OnInit, OnChanges, OnDestroy {
  ...
  @ViewChildren('questionsReference') questionsReference: QueryList<QuestionComponent>;
  ...
  ngOnChanges(changes: SimpleChanges) {
    for (const change in changes) {
      if (change === 'quiz') {
        if (this.processedContent.quiz && this.processedContent.quiz.timeLimit) {
  ...
  startQuiz() {
    ...
    if (this.processedContent.quiz.timeLimit > -1) {
      this.timerSubscription = interval(100)
        .pipe(map(() => this.startTime + this.processedContent.quiz.timeLimit - Date.now()))
  ...
  proceedToSubmit() {
    if (this.timeLeft) {
      if (Object.keys(this.questionAnswerHash).length !== this.processedContent.quiz.questions.length) {
  ...
  calculateResults() {
    const correctAnswers = this.processedContent.quiz.questions.map(question => {
      return {
      ...
      if (
        ...
        if (selectedOptions && selectedOptions[0]) {
          ...
          selectedOptions[0].forEach(element => {
            ...
            if (correctMtfOptions[(b.slice(-1) as number) - 1].match.trim() === element.target.innerHTML.trim()) {
    ...
    this.numUnanswered =
      this.processedContent.quiz.questions.length - this.numCorrectAnswers - this.numIncorrectAnswers;
  }
