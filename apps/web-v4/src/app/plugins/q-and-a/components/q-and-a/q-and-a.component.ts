...
export class QAndAComponent implements OnInit {
  ...
  paramSubscription;
  ngOnInit() {
    this.paramSubscription = this.activatedRoute.data.subscribe(data => {
      const conv = data.playerDetails.conversation;
      if (conv && conv.mainPost && conv.mainPost.id) {
        this.questionData = conv;
      }
    });
    this.activatedRoute.params.subscribe(params => {
      this.postId = params.resourceId;
  ...
  onAnswerAccept(itemId: string) {
    try {
      const itemIndex = this.questionData.replyPost.findIndex(reply => reply.id === itemId);
      const pullItem = this.questionData.replyPost.splice(itemIndex, 1);
      let replyItem: ITimelineResult;
      if (this.questionData.acceptedAnswer && this.questionData.acceptedAnswer.id) {
      ...
      console.error('ERROR WHILE FILLING ACCEPTED ANSWER IN CACHE DATA. RE-FETCHING CONVERSATION DATA');
      this.fetchConversationData(true);
    }
  }
  ...
  onDeleteSuccess(data: { isAccepted: boolean; id: string }) {
    try {
      if (!data.isAccepted) {
        const itemIndex = this.questionData.replyPost.findIndex(reply => reply.id === data.id);
