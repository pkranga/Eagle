...
export class DndQuizService {
  ...
  private initQuiz(quizData: IDndQuiz) {
    ...
    quizData.dndQuestions.options.answerOptions.forEach((option: IDndSnippet) =>
  ...
  submit(quizData: IDndQuiz, answerList: IDndSnippet[]) {
    ...
    const answer: IDndQuiz = {
      ...
      dndQuestions: {
        ...
        options: {
          answerOptions: answerList.filter(option =>
            answerSet.has(option.id)
          ),
          additionalOptions: answerList.filter(option =>
            additionalSet.has(option.id)
          )
        }
      }
    };
  ...
  private evaluate(quizData: IDndQuiz, answer: IDndQuiz): boolean {
    ...
    if (
      answer.dndQuestions.options.additionalOptions &&
      answer.dndQuestions.options.additionalOptions.length
    ) {
      result = false;
      return result;
    }
    ...
    if (
      (!answer.dndQuestions.options.answerOptions.length &&
        quizData.dndQuestions.options.answerOptions.length) ||
      (answer.dndQuestions.options.answerOptions.length !==
        quizData.dndQuestions.options.answerOptions.length)
    ) {
      result = false;
    ...
    answer.dndQuestions.options.answerOptions.forEach(option => {
      ...
      if (option.hasDropDown) {
        ...
        option.ddOptions.forEach(menu => {
          ...
          const correctDDOption = keyOption.ddOptions[j].ddownOptions.find(
            ddOption => ddOption.isCorrect
          );
          ++j;

          if (
            !(selectedDDOption && correctDDOption) ||
            !(selectedDDOption === correctDDOption.text)
          ) {
            result = false;
            return result;
          }
