...
import { AfterViewInit, Component, ElementRef, OnChanges, OnDestroy, OnInit, Renderer, ViewChild } from '@angular/core';
...
import { IClassDiagramResponse, IOptionsObject } from '../../model/classDiagram.model';
  ...
  submit() {
    ...
    if (classElements.length) {
      for (let i = 0; i < classElements.length; i++) {
        ...
        const className = exists.length ? exists[0].innerHTML.trim() : '';
        ...
        if (attr.length) {
          for (let j = 0; j < attr.length; j++) {
            const attributeValue = attr[j].innerHTML.trim();
            // tslint:disable-next-line:max-line-length
        ...
        if (method.length) {
          for (let j = 0; j < method.length; j++) {
            const methodValue = method[j].innerHTML.trim();
            // tslint:disable-next-line:max-line-length
            const accessSpecifier =
      ...
      this.jsPlumbInstance.select().each(info => {
        ...
        if (sourceClass.length && targetClass.length) {
          const object = {
            source: sourceClass[0].innerHTML.trim(),
            relation: info.getOverlay('label').getLabel(),
            target: targetClass[0].innerHTML.trim()
          };
