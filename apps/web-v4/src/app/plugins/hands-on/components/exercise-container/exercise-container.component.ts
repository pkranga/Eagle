...
export class ExerciseContainerComponent
    ...
    if (this.exerciseData.timeLimit > -1) {
      this.timerSubscription = interval(100)
        .pipe(
          map(
            () =>
              this.exerciseStartedAt + this.exerciseData.timeLimit - Date.now()
          )
        )
  ...
  verify() {
    ...
    if (this.processedContent.handsOn.forFPCourse) {
      ...
      this.handsOnSvc.verifyFp(lexId, exerciseDataFP).subscribe(data => {
        if (data) {
      ...
      this.handsOnSvc.verifyCe(lexId, exerciseDataCE).subscribe(data => {
        if (data) {
          this.verifyData = { ...data };
          this.verifyResult = JSON.parse(this.verifyData.verifyResult);
  ...
  submit(ignore_error = false) {
    ...
    if (this.processedContent.handsOn.forFPCourse) {
      ...
      this.handsOnSvc.submitFp(lexId, exerciseDataFP).subscribe(data => {
        if (data) {
      ...
      this.handsOnSvc.submitCe(lexId, exerciseDataCE).subscribe(data => {
        if (data) {
          this.submitData = { ...data };
          this.submitResult = this.submitData.submitResult;
          if (!this.submitResult.submitionStatus) {
      ...
      (data) => {
        if (data) {
          if (data === '---no submission found---') {
            ...
            const viewLastSubmissionlData: string = <string>data;
            this.exerciseData.starterCodes[0] = viewLastSubmissionlData;
          }
        }
      },
      (error) => {
  ...
  copyToClipBoardFunction() {
    ...
    let existsTextarea = <HTMLTextAreaElement>document.getElementById(id);
    if (!existsTextarea) {
