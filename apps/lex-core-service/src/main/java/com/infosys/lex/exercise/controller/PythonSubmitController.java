...
@RestController
@CrossOrigin(origins = "*")
public class PythonSubmitController {
	...
	@Autowired
	public PythonSubmitController(SubmitService submitServ, @Qualifier("pythonVerification")VerificationService verifyServ) {
	 ...
	 * @throws Exception
	 */
	@PostMapping("/v1/users/{user_id}/exercises/{exercise_id}/python-submission")
	public ResponseEntity<Map<String, Object>> pythonVerification(
		...
		if (action.toLowerCase().equals("submit")) {
			...
			output.put("id", "api.exercise.python.submit");
			...
			output.put("submitResult", submitServ.submit(rootOrg,verifySummary, toSubmit, userId, exerciseId, "python"));

		} else {
			output.put("id", "api.exercise.python.verify");
